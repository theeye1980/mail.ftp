...
$query = "select id,vendorCode from goods where `push_in_ozon`=1";
...
foreach($arr_all_xml['file'] as $url_xml){
    ...
    // Пропускаем из обработки Arlight
    if ($vendor == "Arlight") continue;
     
    echo " отображаем поставщика ". $vendor .' - ' . $vendor_code . "<br>";
