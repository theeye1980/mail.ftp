...
class rec_bd{
	...
	function get_goods_to_push(){ # функция получения артикулов, которые не добавлены в озон и не имеют task_id
		...
		$sql="SELECT * FROM `goods` WHERE `ozon_product_id`=0 and `task_id` = 0 and `push_in_ozon`=1";
		echo $sql;
		$statement = $modx->query($sql);
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);
		$i=0;
	...
	function rec_1c_add_order($id){ # записываем в таблицу orders_ozon_1с информацию о том, что заказ прокинут в 1С
	    global $modx;
		
		$query = "INSERT INTO `orders_ozon_1с` (`ozon_id`, `in1c`) VALUES ('$id','1')";
		$modx->query($query);
...
class ozon_api{
    ...
    var  $get_order_info_url='/v1/order/'; // для получения информации о конкретном заказе
    
    function get_order_info($order_num){ # получаем информацию по заказу
        
        echo "Привет от старых щиблет";
        #Формируем адрес запроса
        $comand_url=$this->get_order_info_url . $order_num . '?translit=true';
        echo $comand_url;
        $response .= $this->curl_ozon_get($comand_url);
    ...
    function curl_ozon_get($comand_url){
            # Формируем конечный URL, куда стучимся
            $url=$this->url1 . $comand_url;
...
class api_1c{ 
    ...
    var $set_order='/br/hs/fandeco/order_send'; # Запрос на размещение в 1С заказа;
    
    function set_order($num_order,$order_set){ # Записывем в 1С данные по заказу
        global $modx;
        # $num_order - номер заказа в озоне
        # $order_set - массив с данными о заказе, список товаров, количество и цены
         print_r($order_set);
        
        
        # Формулируем продукты для json
        $product_part_json='';
        
        
        foreach($order_set as $product){
        $needle=$product['offer_id'];
        $index=array_search($needle,$tables);    
           echo $product['offer_id'] . '  ' .  $product['quantity'] . " - продукта, индекс - $index" . "<br>";
           
           $ozon_product_id=$product['product_id'];
           
           # узнаем артикул 1С
           $sql="select article1c from goods where `ozon_product_id`='$ozon_product_id'";
           $statement = $modx->query($sql);
           $result = $statement->fetchAll(PDO::FETCH_ASSOC);
           
           foreach ($result as $r){
               $article[0] = $r['article1c'];
           }
           
            
           $product_part_json  .=  $modx->getChunk('product_item_1C',array(
                                                    'article' => $article[0],
        ...
        $note='Заказ с Озон №' . $num_order;
        
        $json=$modx->getChunk('order_set_1c',array(
                                'product_items'=>$product_part_json,
                                'order_id'=>$num_order,
                                'note'=>$note,
            ));
        echo $json;
        
        # стучимся в 1С с данными заказа
        
         $result = $this->base_ping($this->set_order,$json);
        return $result;
    }
    ...
    function base_ping($comand_url,$json){
        ...
        # Формируем конечный URL, куда стучимся
        $url=$this->url1 . $comand_url;
        $auth=$this->userpwd . ':' . $this->userpwd;
        
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array("Content-Type: application/json, charset=UTF-8"));
        curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
        curl_setopt($ch, CURLOPT_USERPWD, "webuser:4G47r_3");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $json);
